...
- SillyTavern can call `https://scene-worker.gamedesign.152.53.117.246.sslip.io/render/from-excerpt` from a browser extension; the worker already serves the final artifacts itself.
- The same extension now also has an `SD Generate` button that posts the latest excerpt to the n8n `sd-agent` webhook and shows the returned image.
...
- The scene render button now uses the worker's queue/poll API instead of one long blocking render call.
