...
## Decision
Import the image as a new scene asset named `wb_speedup_succubus.png`, compress it down to a smaller square version, and replace the placeholder drawing in `WorldBuildingScene` with the real image anchored to the floor line.
...
- Resized the image to `800x800` for a smaller repo asset.
